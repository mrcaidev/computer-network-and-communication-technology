...
- [项目一阶段三报告](#项目一阶段三报告)
  ...
  - [三、网络层（主机）](#三网络层主机)
    ...
    - [3.8 代码框架](#38-代码框架)
    - [3.9 阶段二调试](#39-阶段二调试)
      - [3.9.1 Unicode字符串I/O](#391-unicode字符串io)
      - [3.9.2 图片的读写](#392-图片的读写)
      - [3.9.3 差错检测与重传](#393-差错检测与重传)
      - [3.9.4 Keepalive机制的触发](#394-keepalive机制的触发)
      - [3.9.5 吞吐量测试](#395-吞吐量测试)
  - [四、网络层（交换机）](#四网络层交换机)
...
具体代码可以在`utils/coding.py`中找到，该模块留下6个API供外部调用：
...
这一操作被封装在了`utils.layer.NetLayer`类内，具体的代码可以在`utils/layer.py`中找到。
...
理论上，这一长度是没有上限的；因为我们编写的系统会将单次传输的信息分为多帧，每帧总长均为100余位左右，远远小于1500位MTU（以太网标准）的限制。
...
#### 3.9.4 Keepalive机制的触发

当误码率进一步增大（在本例中设为10000，即10%），发送端的Keepalive机制将被触发，会停止重传，用户需要重新发送信息。

![Keepalive](images/stage2-4-1.png)

当信道环境恢复正常（在本例中为20，即0.02%），用户重新输入的信息就可以再次发送，接收端也可以正确接收。
...
![网速测试](images/stage2-5-1.png)

两边的物理层误码率从0到1000每100为一档，每档测试3次取平均值。使用`Python+Matplotlib`绘制出测试结果曲线如下：

![阶段二调试4-2](images/stage2-5-2.png)

在信道环境良好的情况下，吞吐量可以达到1800bps左右；该系统对误码率的承受程度约为1%。
...
测试方法与[3.9.4节](#394-吞吐量测试)基本一致，但本次测试调整的是交换机两个物理层端口的误码率，从0到1000，每100为一档。

![阶段三调试3-1](images/stage3-3-1.png)

同样绘制曲线如下：

![阶段三调试3-2](images/stage3-3-2.png)

左边是[3.9.4节](#394-吞吐量测试)的曲线，作为对照；右边是本次有交换机的吞吐量测试结果。在信道环境良好的情况下，吞吐量可以达到800bps左右；该系统对误码率的承受程度约为1%。
...
2. 在高误码率、传输长信息的情况下，会出现接收端在接收完之后，自动成为发送端并发送相同信息的bug。我们尚未定位到错误来源。
