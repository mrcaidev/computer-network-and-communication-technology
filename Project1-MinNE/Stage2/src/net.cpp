    ...
    while (true) {
        ...
        if (mode == RECV_MODE) {
            ...
            for (int frame = 0; frame < recvTotal + reRecvNum; ++frame) {
                ...
                // 如果当前序号与上一序号重复，就跳过；否则更新接收序号。
                if (recvSeq == recvFrame.getSeq()) {
                    reRecvNum++;
                    continue;
                } else {
                    recvSeq = recvFrame.getSeq();
                }
                // 验证校验和。
                cout << "Frame[" << recvSeq << "] ";
                if (recvFrame.isVerified()) {
                    ...
                    Frame ack(appPort, recvSeq, encode(ACK),
                    ...
                    reRecvNum++;
                    Frame nak(appPort, recvSeq, encode(NAK),
                              recvFrame.getSrcPort());
            ...
            Frame request(appPort, sendSeq, decToBin(sendTotal, SEQ_LEN),
                          dstPort);
            ...
            for (int frame = 0; frame < sendTotal; ++frame) {
                ...
                // 把取出来的消息封装进帧。
                Frame readyFrame(appPort, sendSeq, thisMessage, dstPort);
                packages[frame] = readyFrame;
            }
            // 所有消息封装完成，逐帧发送。
            for (int frame = 0; frame < sendTotal; ++frame) {
                // 如果是第一次发这个帧，就先序列化并打印；重传的话就不用再做一遍了。
                if (isFirstSend) {
